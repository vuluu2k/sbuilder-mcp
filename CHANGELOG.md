...
### Added
- `sb_review` now reports `blogCategoryPage` and `coursePage` readiness gaps when a site has blog categories or courses but no published page of the `blog` or `course` type, since `/blog-categories/{slug}` and `/courses/{slug}` fall back to those templates and every entity link 404s without them, completing the check against all five entity prefixes the storefront registers.
...
### Fixed
- sb_remove's dry run now reports removing as an actual node count instead of the total patch count, which was off by one in the ordinary case because deleting a subtree also emits one extra patch to unlink it from its parent's child list; the raw patch count is still available under the new patches field.
