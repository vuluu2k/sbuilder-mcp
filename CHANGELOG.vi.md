...
### Fixed
- Bảng `page_types` của `sb_page_list` giờ kèm `page_types_note` nói rõ mười hai loại trang là các địa chỉ cố định mà storefront định tuyến tới, không phải danh sách các trang một site cần có, vì login, register, contact, about, các trang chính sách và bất cứ điều gì shop cần nói đều là các dòng `page` thông thường, không giới hạn số lượng.
...
### Added
- `sb_page_create` giờ mở trang Giới thiệu, chính sách hoặc FAQ với một layout thực sự thay vì canvas trống, được xác định bằng cách so khớp `name` của trang với đúng bảng từ khoá mà `sb_page_list` đã dùng để báo thiếu trang; kết quả trả về `would_open_as` (khi dry run) hoặc `seeded: { layout, nodes }` (khi chạy thật) trong trường hợp này, và `seed:false` vẫn dùng để bỏ qua tính năng này.
...
### Added
- `sb_page_list` giờ báo cáo site này có vẻ đang thiếu những trang mà hầu hết các site thường có — đăng nhập, đăng ký, quên mật khẩu, liên hệ, giới thiệu và các trang chính sách — bằng cách so khớp theo tên với các trang loại `page` không có kiểu riêng, kèm ghi chú nói rõ đây là gợi ý chứ không phải lỗi.
