...
`sb_set` also warns when the KEY itself is read by nothing. The vocabularies below answer
"this value means something other than you think"; this answers "no value means anything". An
element seeds the key, `sb_node_read` returns it with a plausible value, and a write to it
stores, saves, publishes and renders exactly as before, with no error at any step. The
inspector draws no row for such a key, so only an agent can reach it. Generated by scanning
every identifier the platform ships — 384 seeded keys against 76,235 identifiers in 3,966
files, one of them dead (`config.splitDirection` on `image-comparison`). A key read ONLY by
the editor is correct and is never reported. The note names the seed site, because the fix is
upstream and no spelling of the key works.
...
`specials_values` is the same answer for the `specials` namespace, split out because that is
the namespace you write to. Every entry in both names its own `writeKey`, and reading it is not
optional: the entry is keyed by the CONTROL wherever the editor's picker is the source, and a
control's name is NOT its key — `divider_orientation` writes `config.orientation`, and eleven of
the thirteen joined controls are like it. Two entries also carry a flag worth knowing: `open`
means the renderer hands anything unlisted straight through (`mediaImageRatio` really does take
a verbatim `4 / 5`, so the listed values are the ones with SPECIAL meaning rather than the only
legal ones), and a MISSING `fallback` means the source did not say — the editor's picker proves
what an author may choose and is silent on what the renderer does with anything else.

**Scoped to the element, and that is not a detail.** `config.layout` means one set of words on
`media-dataset` and another on `list-dataset`; `specials.source` is written by `breadcrumb_source`
(`auto|manual`) and by `qr_source` (`text|page`); three renderers read `config.placement` with
three different case sets. A table keyed by the write key alone hands one element another's
vocabulary, which is the same silent wrong answer the table exists to prevent. Nothing is
published unless its source PROVES the list complete — a Go `switch` whose `default:` arm catches
everything the cases do not, or the editor picker's own option list. An equality comparison
proves neither, so those keys are absent rather than half-answered: a partial list would call a
working value invalid, and an agent that believed it would "fix" a page that was already right.
...
unless every row of the menu of that name is `type:"none"`, like the four placeholders `sb_menu`
seeds — judged on the stored link types, never on a lookup that may have failed; that one's rows are replaced with the real ones (`would:"fill"` in the dry run,
