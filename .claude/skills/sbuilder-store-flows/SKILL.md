...
- **`action:"chrome"` BUILT ITS NAVIGATION AS A ROW OF BUTTONS, and `sb_review` flagged it as
  ...
  duplicate — EXCEPT a menu whose every row is `type:'none'` (`menuSnapshot`'s `unlinked` = all
  rows, e.g. `sb_menu`'s four placeholders; NEVER `unresolved`, because `resolveLinks` reads a
  failed GET as "no match" and a 5xx would overwrite a real menu with no undo), whose rows are PUT
